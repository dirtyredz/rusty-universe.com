...
class RustyAnnouncments extends React.Component {
  ...
  componentDidMount() {
      const Int = setInterval(()=>{
        if(this.state.LoadingMessage === "Loading..."){
    ...
    fetch(
      "https://7vo29h6nnd.execute-api.us-east-2.amazonaws.com/production/DirtyBot"
    )
      .then(response => {
    ...
    return (
      <AnnouncmentsContainer style={{ height: 300 }}>
        {this.state.Announcments
          ? this.state.Announcments.map(m => {
              ...
              Lines = Lines.map(line => {
                ...
                Words = Words.map(word => {
                  ...
                  if (onlyUrl) {
                    return (
                      <DiscordATag target="_blank" href={onlyUrl}>
                        {onlyUrl}
                      </DiscordATag>
                    );
                  }
          ...
          : 
            <Announcment style={{fontSize: 30, textAlign: "center"}}>
